...
const deckModel = require("../../models/decks/model.js");
...
const userSchema = (sequelize, DataTypes) => {
  ...
  const model = sequelize.define("User", {
    ...
    // Add a foreign key reference to the Deck model
    deckId: {
      type: DataTypes.INTEGER,
      allowNull: true,
      references: {
        model: deckModel(sequelize, DataTypes),
        key: "id",
      },
    },
  ...
  model.authenticateToken = async function (token) {
    try {
      ...
      }); // find user by username in token
      if (user) {
  ...
  // Define the association between User and Deck models
  model.hasOne(deckModel(sequelize, DataTypes), { foreignKey: "userId" });
